...
lsH = TreeHandler(path_to_data + "HypDataTable_ls.root", "DataTable", entry_stop=1e6)
...
pu.plot_distr([signalH, dataH], column=column,labels=["Sig", "Bkg"], log=True, density=True, figsize=(20, 20), alpha=0.3, grid=False)
pdf.savefig()

pu.plot_distr([lsH, emH],column=column,labels=["LS", "EM"], log=True, density=True, figsize=(20, 20), alpha=0.3, grid=False)
pdf.savefig()

pu.plot_distr([dataH, lsH],column=column,labels=["Data", "LS"], log=True, density=True, figsize=(20, 20), alpha=0.3, grid=False)
pdf.savefig()
